  ...
  return (
    <div className="mb-12 px-2 md:px-12 relative flex justify-between items-start">
      ...
      {pasos.map((paso, idx) => (
        <div
          key={idx}
          className="flex flex-col items-center text-center max-w-[80px] md:max-w-[150px]"
        >
          <div
            className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center mb-2 md:mb-3 z-10 border-2 text-[10px] md:text-[14px] ${
              paso.completado
          ...
          >
            {paso.completado ? (
              <Check size={14} weight="bold" className="md:w-[18px] md:h-[18px]" />
            ) : (
              idx + 1
            )}
          </div>
          <p
            className={`text-[10px] md:text-[14px] font-bold mb-0.5 md:mb-1 leading-tight ${paso.completado || paso.actual ? "text-[#1e1e1e]" : "text-[#8e908f]"}`}
          >
            {paso.label}
          </p>
