...
export function QuoteStatusBanner({ status }: QuoteStatusBannerProps) {
  ...
  return (
    <div
      className={`mb-10 bg-white rounded-[16px] border border-[#d1d1d1] shadow-[0_8px_30px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col md:flex-row`}
    >
      <div className="flex-1 p-6 md:p-8 flex items-center gap-6">
        <div
          className={`w-14 h-14 rounded-full ${config.bgColor} flex items-center justify-center shrink-0 ${config.iconColor}`}
        >
          {config.icon}
        </div>
        <div>
          <h2 className="text-[22px] font-bold text-[#1e1e1e] mb-1">{config.title}</h2>
          <p className="text-[17px] text-[#575757] font-medium">{config.desc}</p>
        </div>
      </div>
      <div className="bg-[#f9f9f9] p-6 md:p-8 flex flex-col justify-center border-t md:border-t-0 md:border-l border-[#e8e8e8] min-w-[320px]">
        <p className="text-[14px] font-bold text-[#1e1e1e] mb-2 uppercase tracking-widest">
          Paso siguiente:
        </p>
        <p className="text-[16px] text-[#575757] mb-5 font-medium">{config.nextStep}</p>
        <button
          className={`w-full h-[52px] ${config.btnColor} text-white font-bold rounded-[10px] transition-all shadow-sm`}
        >
          {config.btnText}
        </button>
      </div>
