...
export const HeaderSection = ({ branch, quotation }: HeaderProps) => {
  ...
  return (
    <View style={styles.header}>
      <View style={styles.companyInfo}>
        ...
        <Text style={{ fontSize: 10, fontWeight: 'bold', marginBottom: 2 }}>{branch?.nombre_sucursal || "Sucursal Principal"}</Text>
      ...
      <View style={[styles.headerInfo, { alignItems: 'flex-end' }]}>
        ...
        <View style={{ marginTop: 8, width: 140, textAlign: 'left' }}>
          <Text style={[styles.branchText, { textAlign: 'left' }]}>
            Fecha: {new Date(quotation.fecha_aprobacion || quotation.fecha_validacion || quotation.fecha_creacion).toLocaleDateString('es-MX')}
          </Text>
          <Text style={[styles.branchText, { textAlign: 'left' }]}>Orden: {quotation.folio || `OT-${quotation.id_cotizacion}`}</Text>
          ...
          <Text style={[styles.branchText, { textAlign: 'left' }]}>E-mail: laser@geekdesign.mx</Text>
        </View>
      </View>
    </View>
  );
