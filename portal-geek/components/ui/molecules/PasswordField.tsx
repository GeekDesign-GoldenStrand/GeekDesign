...
export function PasswordField({
  ...
  return (
    <label className="block w-full">
      ...
      {/* The icons are absolutely positioned relative to THIS wrapper, not the
          label, so their `top-1/2` stays anchored to the input's center even
          when the error message below changes the label's height. */}
      ...
      {error && (
        <p
          id={errorId}
          role="alert"
          className="mt-2 px-6 text-[14px] tracking-[0.5px] text-[#df2646]"
        >
          {error}
        </p>
      )}
