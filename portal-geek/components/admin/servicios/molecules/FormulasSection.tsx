...
export function FormulaSection({
  ...
  const panelBtnOff = "bg-gray-100 text-gray-400 border-gray-200 cursor-default";

  const hasAnyTokens = variables.length > 0 || constantes.length > 0 || terceros.length > 0;
    ...
    <div className="flex flex-col gap-3">
            ...
            if (chunk.type === "text") {
              ...
              return (
                <input
                  ...
                  style={{
                    width: chunk.value.length > 0 ? `${chunk.value.length + 0.5}ch` : "1.5ch",
                  }}
      ...
      <div className="flex flex-col gap-2 p-2.5 bg-blue-50 border border-blue-200 rounded-md">
        <p className="text-sm text-blue-700">
          Escribe <code className="bg-blue-100 px-1 rounded">+ - * / ( )</code> directo, o usa los
          botones de abajo.
        </p>

        {variables.length > 0 && (
          <div className="flex flex-wrap gap-1 items-center">
            <span className="text-sm text-blue-500 font-semibold mr-0.5">Variables:</span>
            {variables.map((v) => (
              <button
        ...
        {constantes.length > 0 && (
          <div className="flex flex-wrap gap-1 items-center">
            <span className="text-sm text-blue-500 font-semibold mr-0.5">Constantes:</span>
            {constantes.map((c) => {
              ...
              return (
                <button
                  ...
                  title={alreadyIn ? "Ya en la fórmula" : isGlobal ? "Constante fija" : undefined}
        ...
        {terceros.length > 0 && (
          <div className="flex flex-wrap gap-1 items-center">
            <span className="text-sm text-blue-500 font-semibold mr-0.5">Terceros:</span>
            {terceros.map((t) => {
        ...
        <div className="flex flex-wrap gap-1 items-center">
          <span className="text-sm text-blue-500 font-semibold mr-0.5">Operadores:</span>
          {OPERATORS.map((op) => (
            <button
              key={op}
              type="button"
