...
export function TopClientesCard({ rows, totalScope, year, availableYears, onYearChange }: Props) {
  const yearOptions = [
    ...
    <div className="bg-white p-5 sm:p-8 rounded-3xl shadow-sm border border-gray-100">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 sm:mb-8 gap-3 sm:gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">Top 10 Clientes</h2>
            <p className="text-xs sm:text-sm text-gray-500 mt-1">
              Ranking por ingresos {year === "all" ? "históricos" : `en ${year}`}
            </p>
          </div>
        </div>
        <SelectField
      ...
      {rows.length === 0 ? (
        ...
        <ul className="space-y-3">
          {rows.map((c, i) => {
            const pct = maxMonto > 0 ? (c.monto / maxMonto) * 100 : 0;
            const share = totalScope > 0 ? (c.monto / totalScope) * 100 : 0;
            return (
              <li
                key={c.id_cliente}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">
                    {c.empresa ? `${c.empresa} · ` : ""}
                    {c.numPedidos} {c.numPedidos === 1 ? "pedido" : "pedidos"} · {share.toFixed(1)}%
                    del total
                  </p>
