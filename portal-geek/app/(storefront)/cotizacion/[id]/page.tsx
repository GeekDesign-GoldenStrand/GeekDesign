...
export default function CotizacionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Convert to lowercase so "validada" or "Validada" works the same
            ...
            {data.estado_actual !== "en_revision" && (
              <div className="mt-8 p-6 bg-blue-50 border border-blue-100 rounded-[12px] flex items-start gap-4">
                ...
                <p className="text-[14px] text-blue-900 leading-relaxed">
                  Al confirmar esta cotización, aceptas los cambios realizados en los servicios
                  marcados como{" "}
                  <span className="font-bold underline decoration-blue-400">modificados</span> y
                  reconoces que los servicios{" "}
                  <span className="font-bold underline decoration-red-400 text-red-700">
                    rechazados
                  </span>{" "}
                  no formarán parte del pedido final.
                </p>
              </div>
            )}
          </div>
          ...
          <QuoteSummary
            ...
            counts={{
              aprobados: data.servicios.filter((s: QuotationItem) => s.estado === "sin_cambios")
                .length,
              modificados: data.servicios.filter((s: QuotationItem) => s.estado === "modificado")
                .length,
              rechazados: data.servicios.filter((s: QuotationItem) => s.estado === "rechazado")
                .length,
            }}
            ...
            actionText={
              data.estado_actual === "en_revision"
                ? "Revisión en curso"
                : data.estado_actual === "modificada"
                  ? "Revisar y confirmar cambios"
                  : data.estado_actual === "rechazada"
