...
export const GET = withSection("cotizaciones", "read", async (req: NextRequest) => {
  ...
  // Pagination parameters: default page=1, pageSize=13. Clamp the same way every
  // other list route does so `?page=0` (negative skip → Prisma throws) and
  // `?pageSize=Infinity` (bypasses any reasonable cap) can't blow up the list.
