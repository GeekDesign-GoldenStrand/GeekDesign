...
export async function listUsuarios(page: number, pageSize: number) {
  ...
  const [items, total] = await prisma.$transaction([
    prisma.usuarios.findMany({ skip, take: pageSize, select: USER_SELECT, orderBy: { fecha_creacion: "desc" } }),
...
export async function getUsuario(id: number) {
  const usuario = await prisma.usuarios.findUnique({ where: { id_usuario: id }, select: USER_SELECT });
...
export async function updateUsuario(id: number, data: UpdateUsuarioInput) {
  try {
    ...
    if ((err as { code?: string }).code === "P2025") throw new NotFoundError("Usuario no encontrado");
    throw err;
  }
}
